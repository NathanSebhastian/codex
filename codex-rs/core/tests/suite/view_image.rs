...
async fn js_repl_poll_view_image_tool_attaches_local_image() -> anyhow::Result<()> {
    ...
    let mut builder = test_codex().with_config(|config| {
        config.features.enable(Feature::JsRepl);
        config.features.enable(Feature::JsReplPolling);
    });
    let TestCodex {
        codex,
        cwd,
...
"#
    ...
    struct PollSequenceResponder {
        ...
        poll_call_id_prefix: &'static str,
        js_input: String,
    }
    ...
    impl Respond for PollSequenceResponder {
        fn respond(&self, request: &wiremock::Request) -> ResponseTemplate {
            ...
            let input = request_json["input"].as_array();
            let body = if let Some(submit_output) = input.and_then(|items| {
                items.iter().find(|item| {
                    item.get("type").and_then(Value::as_str) == Some("custom_tool_call_output")
                        && item.get("call_id").and_then(Value::as_str) == Some(self.submit_call_id)
                })
            }) {
                ...
                let exec_id = submit_json["exec_id"]
                    ...
                    .to_string();
                *self.exec_id.lock().expect("exec id lock") = Some(exec_id.clone());
                *self.poll_call_count.lock().expect("poll call count lock") = 1;
                let poll_args = serde_json::json!({
                    "exec_id": exec_id,
                    "yield_time_ms": 5000,
                })
                .to_string();
                let poll_call_id = format!("{}-1", self.poll_call_id_prefix);
                sse(vec![
                    ev_response_created("resp-2"),
                    ev_function_call(&poll_call_id, "js_repl_poll", &poll_args),
                    ev_completed("resp-2"),
                ])
            } else if let Some(poll_output) = input.and_then(|items| {
                items.iter().find(|item| {
                    item.get("type").and_then(Value::as_str) == Some("function_call_output")
                        && item
                            .get("call_id")
                            .and_then(Value::as_str)
                            .is_some_and(|call_id| call_id.starts_with(self.poll_call_id_prefix))
                })
            }) {
                let output = poll_output
                    .get("output")
                    .expect("poll output should be present");
                let status = match output {
                    Value::String(text) => serde_json::from_str::<Value>(text)
                        .expect("poll text output should be valid json")["status"]
                        .as_str()
                        .expect("poll text status should be present")
                        .to_string(),
                    Value::Array(items) => serde_json::from_str::<Value>(
                        items
                            .first()
                            .expect("poll content output should include status item")["text"]
                            .as_str()
                            .expect("poll status item text should be present"),
                    )
                    .expect("poll content status should be valid json")["status"]
                        .as_str()
                        .expect("poll content status should be present")
                        .to_string(),
                    other => panic!("unexpected js_repl_poll output shape: {other:?}"),
                };
                if status == "running" {
                    let exec_id = self
                        .exec_id
                        ...
                        .expect("exec id lock")
                        .clone()
                        .expect("exec id should be captured from submit output");
                    let poll_call_id = {
                        let mut count = self.poll_call_count.lock().expect("poll call count lock");
                        *count += 1;
                        format!("{}-{}", self.poll_call_id_prefix, *count)
                    };
                    let poll_args = serde_json::json!({
                        "exec_id": exec_id,
                        "yield_time_ms": 5000,
    ...
    Mock::given(method("POST"))
        ...
        .respond_with(PollSequenceResponder {
            ...
            poll_call_count: Arc::new(Mutex::new(0)),
            submit_call_id,
            poll_call_id_prefix: poll_call_id,
            js_input,
        })
        .up_to_n_times(7)
        .mount(&server)
    ...
    assert!(
        (3..=7).contains(&requests.len()),
        "expected submit, poll, and completion requests, with at most four extra running polls"
    );
        ...
        .find(|item| {
            item.get("type").and_then(Value::as_str) == Some("function_call_output")
                && item
                    .get("call_id")
                    .and_then(Value::as_str)
                    .is_some_and(|call_id| call_id.starts_with(poll_call_id))
        })
